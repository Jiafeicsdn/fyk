...
public class FaXianParseEmojiMsgUtil {
...
//                break;
    ...
    public static void getExpressionString(Context context, String messageId, TextView textView, String str,String topicStr,String huaTiId) {
        SpannableString spannableString = new SpannableString(str);
        Pattern sinaPatten = Pattern.compile(REGEX_STR, Pattern.CASE_INSENSITIVE);
        try {
            FaXianImageLoaderEmoji.getInstance().bindBitmap(messageId, spannableString, textView, sinaPatten, 0,topicStr, huaTiId,context);
        } catch (Exception e) {
            Log.e(TAG, e.getMessage());
        }
    }

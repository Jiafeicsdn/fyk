package com.lvgou.distribution.activity.image;
...
public class ImageLoaderConfig {
	 ...
	 * 
	 * @param
	 *            ：显示默认的加载图片 false：不显示默认的加载图片  // bg="#FFFFFF";   Color.parseColor(bg)
	 * @return
	 */
	public static DisplayImageOptions initDisplayOptions(boolean isShowDefault) {
		//String  bg="#FFFFFF";  context.getResources().getColor(R.color.lmh_image_color)  //(R.drawable.bg_image_loading
...
//			bg_image_loading
//			img_search_default
			displayImageOptionsBuilder.showStubImage(R.mipmap.bg_none_good);
			
			//displayImageOptionsBuilder.s
			// 地址为空的默认显示图片
			displayImageOptionsBuilder
					.showImageForEmptyUri(R.mipmap.bg_none_good);
			// 加载失败的显示图片
			displayImageOptionsBuilder.showImageOnFail(R.mipmap.bg_none_good);
		}
		// 开启内存缓存
	...
	public static DisplayImageOptions initDisplayOptions(int targetWidth,
		...
		if (isShowDefault) {
			// 默认显示的图片
			displayImageOptionsBuilder.showStubImage(R.mipmap.home_loading);
			// 地址为空的默认显示图片
			displayImageOptionsBuilder
					.showImageForEmptyUri(R.mipmap.home_loading);
			// 加载失败的显示图片
			displayImageOptionsBuilder
					.showImageOnFail(R.mipmap.home_loading);
		}
		// 开启内存缓存
...
//		builder.memoryCache(new UsingFreqLimitedMemoryCache((int) Runtime.getRuntime().maxMemory()/8));
		...
		builder.discCacheSize(150 * 1024 * 1024);

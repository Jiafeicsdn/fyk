package com.lvgou.distribution.route;
...
public class DriveRouteColorfulOverLay {
     ...
     * 添加驾车路线添加到地图上显示。
     */
    public void addToMap(String name,String address) {
        initPolylineOptions();
        try {
            if (mAMap == null) {
                return;
            ...
            addStartAndEndMarker(name,address);
    ...
    private void addStartAndEndMarker(String name,String address) {
        startMarker = mAMap.addMarker((new MarkerOptions())
                .position(startPoint).icon(getStartBitmapDescriptor())
                .title("当前位置"));
        endMarker = mAMap.addMarker((new MarkerOptions()).position(endPoint)
                .icon(getEndBitmapDescriptor()).title(name).snippet(address));
    }
    ...
    private BitmapDescriptor getStartBitmapDescriptor() {
        return BitmapDescriptorFactory.fromResource(R.mipmap.point4);
    }
    ...
    private BitmapDescriptor getEndBitmapDescriptor() {
        return BitmapDescriptorFactory.fromResource(R.mipmap.location_adress_icon);
    }

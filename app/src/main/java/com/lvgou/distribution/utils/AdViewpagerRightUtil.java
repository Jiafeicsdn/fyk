...
import com.lvgou.distribution.adapter.ImageViewPagerAdapter;
...
public class AdViewpagerRightUtil {
    ...
    private AdViewpagerUtil.OnAdPageChangeListener onAdPageChangeListener; //pagechange回调
    private AdViewpagerUtil.OnAdItemClickListener onAdItemClickListener; //点击事件回调
     ...
     * @param urls
     */
    public AdViewpagerRightUtil(Context context, ViewPager viewPager, ArrayList<String> urls) {
     ...
     * @param dotsize
     */
    public AdViewpagerRightUtil(Context context, ViewPager viewPager, TextView tv_num, int dotsize) {
        this.context = context;
        this.viewPager = viewPager;
        this.tv_num = tv_num;
    ...
    public AdViewpagerRightUtil(Context context, ViewPager viewPager, TextView tv_num, int dotsize, int dotoffset, ArrayList<String> urls) {
        this.context = context;
        this.viewPager = viewPager;
        this.tv_num = tv_num;
        this.dotsize = dotsize;
    ...
    public void setDatas(int dotoffset, ArrayList<String> urls) {
        this.urls = urls;
        stopLoopViewPager();
        initVps();
    }

    public void setOnAdPageChangeListener(AdViewpagerUtil.OnAdPageChangeListener onAdPageChangeListener) {
        this.onAdPageChangeListener = onAdPageChangeListener;
    }

    public void setOnAdItemClickListener(AdViewpagerUtil.OnAdItemClickListener onAdItemClickListener) {
        this.onAdItemClickListener = onAdItemClickListener;
    }


    private void initAdimgs(ArrayList<String> urls) {
    ...
    private void setImg(int length, ArrayList<String> urls) {
        if (urls.size() > 0) {
            imgsize = length;
            for (int i = 0; i < length; i++) {
                if (i < length - 2) {
                    final int index = i;
                    Glide.with(context).load(urls.get(i)).into(mImageViews[i + 1]);
            ...
            Glide.with(context).load(urls.get(urls.size() - 1)).error(R.mipmap.pictures_no).into(mImageViews[0]);
            Glide.with(context).load(urls.get(0)).error(R.mipmap.pictures_no).into(mImageViews[length - 1]);
        }
    }
...
//                        isLoop = false;
        ...
        viewPager.setOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int arg0) {
                ...
                tv_num.setText(currentIndex+"/"+urls.size());
    ...
    public void setIsAuto(boolean isLoop) {
        this.isLoop = isLoop;
    }

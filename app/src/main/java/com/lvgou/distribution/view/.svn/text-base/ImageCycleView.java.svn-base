...
public class ImageCycleView extends LinearLayout {
    ...
    public void setImageResources(ArrayList<String> imageUrlList, ImageCycleViewListener imageCycleViewListener) {
        ...
        for (int i = 0; i < imageCount; i++) {
            ...
            mImageView.setLayoutParams(new LayoutParams(30, 30));
            mImageView.setPadding(10, 10, 15, 15);
            mImageViews[i] = mImageView;
            if (i == 0) {
                mImageViews[i].setBackgroundResource(R.mipmap.lunbo_selected);
            } else {
                mImageViews[i].setBackgroundResource(R.mipmap.lunbo_defult);
            }
    ...
    private final class GuidePageChangeListener implements OnPageChangeListener {
        ...
        public void onPageSelected(int index) {
            ...
            mImageIndex = index;
            // 设置图片滚动指示器背景
            mImageViews[index].setBackgroundResource(R.mipmap.lunbo_selected);
...
//          imageView.setTag(imageUrl);
            options_head = new DisplayImageOptions.Builder()
                    .showStubImage(R.mipmap.binner_fail_bg)            // 设置图片下载期间显示的图片
                    .showImageForEmptyUri(R.mipmap.binner_fail_bg)    // 设置图片Uri为空或是错误的时候显示的图片
                    .showImageOnFail(R.mipmap.binner_fail_bg)            // 设置图片加载或解码过程中发生错误显示的图片
                    .build();
            ImageLoader.getInstance().displayImage(imageUrl, imageView, options_head);

...
public class PullToRefreshView extends LinearLayout {
	...
	private void addHeaderView() {
        mHeaderView = new PullToRefreshViewHeader(mContext);
		...
		headerProgressBar=mHeaderView.getHeaderProgressBar();
	...
	private void addFooterView() {
        ...
        mFooterView = new PullToRefreshViewFooter(mContext);
	...
	private void headerPrepareToRefresh(int deltaY) {
		...
		if (newTopMargin >= 0 && mHeaderView.getState() != PullToRefreshViewHeader.STATE_REFRESHING) {
		    //提示松开刷新
		    mHeaderView.setState(PullToRefreshViewHeader.STATE_READY);
		
		} else if (newTopMargin < 0 && newTopMargin > -mHeaderViewHeight) {
		    //提示下拉刷新
		    mHeaderView.setState(PullToRefreshViewHeader.STATE_NORMAL);
		}
		
		/*更新headerview中进度条进度，因为newTopMargin刚开始是负的mHeaderViewHeight高度，当两者相加的值除以mHeaderViewHeight高度就是进度，当两者相加的和超过mHeaderViewHeight高度时，直接把进度条设置到最大*/
		if ((newTopMargin+mHeaderViewHeight)<=mHeaderViewHeight) {
			float progress=Float.parseFloat((newTopMargin+mHeaderViewHeight)+"")/Float.parseFloat(mHeaderViewHeight+"");
			headerProgressBar.setProgress(progress);
			LogUtils.i("newTopMargin值"+newTopMargin+"----"+"headerview高度："+mHeaderViewHeight);
			LogUtils.i("进度值"+progress);
		}else {
			headerProgressBar.setProgress(1.0f);
		}
	...
	private void footerPrepareToRefresh(int deltaY) {
		...
		if (Math.abs(newTopMargin) >= (mHeaderViewHeight + mFooterViewHeight) && mFooterView.getState()  != PullToRefreshViewFooter.STATE_LOADING) {
			mFooterView.setState(PullToRefreshViewFooter.STATE_NORMAL);  //设置释放加载提示
			LogUtils.i("footer view 完全显示");
		} else if (Math.abs(newTopMargin) < (mHeaderViewHeight + mFooterViewHeight)) {
		    mFooterView.setState(PullToRefreshViewFooter.STATE_READY);  //设置上拉加载更多提示
		    LogUtils.i("footer view 还没完全显示");
		}
	}
	...
	public void headerRefreshing() {
		...
		mHeaderView.setState(PullToRefreshViewHeader.STATE_REFRESHING);
	...
	private void footerLoading() {
		mPullLoading = true;
		mFooterView.setState(PullToRefreshViewFooter.STATE_LOADING);  //设置正在加载提示
	...
	public void onHeaderRefreshFinish() {
		...
		if(mAdapterView!=null){
			...
			//判断有没有数据
			if(mCount > 0){
				mFooterView.setState(PullToRefreshViewFooter.STATE_READY);
			}else{
				mFooterView.setState(PullToRefreshViewFooter.STATE_EMPTY);
			}
		}else{
			mFooterView.setState(PullToRefreshViewFooter.STATE_READY);
		}
	...
	public void onFooterLoadFinish() {
		...
		if(mAdapterView!=null){
			...
			if(countNew > mCount){
				...
				mFooterView.setState(PullToRefreshViewFooter.STATE_NO);
			}
		}else{
			mFooterView.setState(PullToRefreshViewFooter.STATE_READY);
		}
...
/*	*//**
	 ...
	 *//*
	int margin;
	public void setAutoRefresh(PullToRefreshView refreshView){
		margin=-mHeaderViewHeight;  //设置默认高值为-mHeaderViewHeight
		LogUtils.i("margin初始值："+margin);
			...
			public void run() {
				UIKit.runOnMainThreadAsync(new Runnable() {
					...
					public void run() {
						...
						LogUtils.i("margin值："+margin);
						setHeaderTopMargin(margin);
						headerPrepareToRefresh(margin);
						if (margin==0) {
							timer.cancel();
						}
...
//		Point point=new Point(0, mHeaderViewHeight);
//		points.add(point);;
//		new PathAnimation(refreshView).setPoints(points).setDuration(2000).animate();
	...
	*//**
	 ...
	 * @param duration 滚动到目标位置的时间
	 *//*
    public void scrollTo(int currentPosition,int toPosition, int duration) {
...
//        int distance = toPosition - currentPosition;
        ...
        mScroller.startScroll(0, currentPosition, 0, toPosition, duration);
    }*/

...
public class AppUtils {
    ...
    public static List<AbAppProcessInfo> getRunningAppProcesses(Context context) {
	    ...
	    try {
	    	...
	    	AppUtils.AbAppProcessInfo abAppProcessInfo = null;
	    	...
	    	for(RunningAppProcessInfo appProcessInfo:appProcessList){
	    		abAppProcessInfo = new AppUtils(). new AbAppProcessInfo(appProcessInfo.processName,appProcessInfo.pid,appProcessInfo.uid);
    ...
    public static List<AbPsRow> ps() {
        ...
        for (String line : lines) {
        	AppUtils.AbPsRow row =new AppUtils(). new AbPsRow(line);
    ...
    public static AbProcessInfo getMemInfo(String processName) {
    	AppUtils.AbProcessInfo process =new AppUtils(). new AbProcessInfo();
    	if(mProcessList == null){
    		mProcessList = getProcessRunningInfo();
    	}
        ...
        if(process.memory == 0){
        	LogUtils.d( "##"+processName+",top -n 1未找到");
        }
        return process;
    }
	...
	public static AbCPUInfo parseCPUInfo(String info) {
		AppUtils.AbCPUInfo CPUInfo = new AppUtils().new AbCPUInfo();
		 String tempString = "";
		 String[] rows = null;
		 String[] columns = null;
		 rows = info.split("[\n]+");
